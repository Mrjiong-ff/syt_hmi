...
class SytRclComm : public QThread {
 ...
 public:
  ...
  template <class T>
  CALL_RESULT callService(std::string srv_name, std::string info,
                          uint32_t timeout_ms,
  ...
  // 上位机参数设置
  bool setNodeParam(const std::string &node_name, const std::string &param_name,
                    const std::string &param_value);
  bool readNodeParam(const std::string &node_name,
  ...
  void loadMachineClothSize(int id, uint32_t width,
                            uint32_t length);               // 裁片尺寸
  ...
  void loadMachineParam(int dtype, std::string field, std::string data,
                        bool is_array);  // 参数设置
  ...
  void composeMachineMoveSucker(
      syt_msgs::msg::ComposeMachineSuckerStates sucker_states);  // 移动吸盘
  ...
  void composeMachineParam(int dtype, std::string field, std::string data,
                           bool is_array);  // 参数设置

  // 缝纫机
  void sewingMachineReset();  // 缝纫机复位
  void sewingMachineMoveHand(float x, float y, float c, bool z);  // 移动抓手
  void sewingMachineSendKeypoints(
      syt_msgs::msg::ClothKeypoints2f keypoints);  // 发送关键点
  void sewingMachineNeedle(float line_1, float line_2, float line_3,
                           float line_4);        // 发送针长
  void sewingMachineThickness(float thickness);  // 缝纫厚度
  void sewingMachineLabelWidth(bool enable, int side, float width,
                               float position);  // 水洗标设置
  void sewingMachineLabelReset();                // 水洗标复位
  void sewingMachineSpeed(int speed);            // 缝纫机档位
  void sewingMachineMode(int mode);              // 转换运行模式
  void sewingMachineParam(int dtype, std::string field, std::string data,
                          bool is_array);  // 参数设置
  ...
  // 样式相关
  void createStyle(int mode, QString prefix,
                   syt_msgs::msg::ClothStyle cloth_style_front,
                   syt_msgs::msg::ClothStyle cloth_style_back);  // 创建样式
 ...
 private:
  ...
  // subscription
  rclcpp::Subscription<syt_msgs::msg::LoadMachineState>::SharedPtr
      load_machine_state_subscription_;
  rclcpp::Subscription<syt_msgs::msg::ComposeMachineState>::SharedPtr
      compose_machine_state_subscription_;
  rclcpp::Subscription<syt_msgs::msg::SewingMachineState>::SharedPtr
      sewing_machine_state_subscription_;
  rclcpp::Subscription<std_msgs::msg::Int32>::SharedPtr download_subscription_;
  rclcpp::Subscription<syt_msgs::msg::LoadClothVisual>::SharedPtr
      load_cloth_visual_subscription_;
  rclcpp::Subscription<rcl_interfaces::msg::Log>::SharedPtr log_subscription_;
  rclcpp::Subscription<syt_msgs::msg::FSMState>::SharedPtr
      run_state_subscription_;
  rclcpp::Subscription<std_msgs::msg::UInt64>::SharedPtr
      run_count_subscription_;
  rclcpp::Subscription<syt_msgs::msg::ErrorCode>::SharedPtr
      error_code_subscription_;

  // publisher
  rclcpp::Publisher<syt_msgs::msg::FSMFlowControlCommand>::SharedPtr
      fsm_flow_control_cmd_publisher_;
  rclcpp::Publisher<syt_msgs::msg::FSMRunMode>::SharedPtr
      fsm_run_mode_publisher_;
 ...
 signals:
  ...
  // 视觉检测相关信号
  void signGetClothInfoFinish(bool result, int cloth_type,
                              syt_msgs::msg::ClothInfo cloth_info);
  void signCheckCalibrationFinish(bool result, float bottom_length,
                                  float side_length);
  ...
  void signGetClothStyleFinish(bool result,
                               syt_msgs::msg::ClothStyle cloth_style_front,
