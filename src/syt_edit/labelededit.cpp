...
LabeledEdit::LabeledEdit(QWidget *parent) : QWidget(parent) {
    ...
    connect(line_edit, &BottomLineEdit::signalFocusIn, this, [=] {
        connect(startAnimation("FocusProg", getFocusProg(), 100, focus_duration, QEasingCurve::OutQuad),
    ...
    connect(line_edit, &BottomLineEdit::signalFocusOut, this, [=] {
        if (line_edit->hasFocus()) // 比如右键菜单，还是算作聚焦的
            return;
        connect(startAnimation("LosesProg", getLosesProg(), 100, focus_duration, QEasingCurve::OutQuad),
                &QPropertyAnimation::finished, this, [=] {
...
void LabeledEdit::showWrong() {
    ...
    connect(startAnimation("WrongProg", wrong_prog, 100, wrong_duration, QEasingCurve::OutQuad),
            &QPropertyAnimation::finished, this, [=] {
                // 只显示波浪线一次
                wrong_prog = 0;
...
//          repaint(loading_rect);
    ...
    loading_timer->start();
    connect(startAnimation("ShowLoadingProg", getShowLoadingProg(), 100, show_loading_duration,
                           QEasingCurve::QEasingCurve::OutBack), &QPropertyAnimation::finished, this, [=] {
...
void LabeledEdit::hideLoading() {
    connect(startAnimation("HideLoadingProg", getHideLoadingProg(), 100, hide_loading_duration, QEasingCurve::OutQuad),
            &QPropertyAnimation::finished, this, [=] {
...
void LabeledEdit::hideMsg() {
    ...
    if (getMsgHideProg() == 0)
        setMsgHideProg(1);
    connect(startAnimation("MsgHideProg", getMsgHideProg(), 100, msg_hide_duration, QEasingCurve::OutQuad),
            &QPropertyAnimation::finished, this, [=] {
                // 只隐藏一次就清空
                msg_hide_prog = 0;
                msg_hiding = "";
        ...
        } else // correct_prog // 显示箭头
        {
            ...
            auto paint2Line = [&](int blank_left, int right_margin) {
                ...
                if (focus_prog || loses_prog) {
                    ...
                    } else // 分为两截画
                    {
                        ...
                        if (blank_left + blank_len < line_right)
                            painter.drawLine(blank_left + blank_len, line_top, qMin(r, line_right - right_margin),
                                             line_top);
                    }
                }
            };
            ...
            } else // 延伸、旋转
            {
                if (correct_prog <= step2) {
                    // 画两截的线
                    int move_dis = (short_len + move_left + blank_len) * (correct_prog - step1) /
                                   (step2 - step1); // 相对于上一阶段的最左边
                ...
                if (angle_turned > 0) {
                    ...
                    QPointF pos1(line_right - radius * 3 / 2 + offset,
                                 line_top - radius + radius / 2 * sin(PI / 3)); // 左下角
                ...
                {
                    QPointF pos2(line_right - radius * 3 / 2 + offset,
                                 line_top - radius + radius / 2 * sin(PI / 3)); // 右下角
...
//            painter.setRenderHint(QPainter::TextAntialiasing, true);
    ...
    } else // 错误曲线
    {
        ...
        painter.setClipRect(QRectF(line_left - pen_width / 2, line_top - ampl - pen_width, line_width + pen_width,
                                   line_top + ampl + pen_width));
...
QPropertyAnimation *
LabeledEdit::startAnimation(QByteArray name, double start, double end, int duration, QEasingCurve curve) {
