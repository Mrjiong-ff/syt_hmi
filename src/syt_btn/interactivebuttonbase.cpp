...
InteractiveButtonBase::InteractiveButtonBase(QWidget *parent)
        : QPushButton(parent), icon(nullptr), text(""), paint_addin(),
          ...
          font_size(0), fixed_fore_pos(false), fixed_fore_size(false), text_dynamic_size(false), auto_text_color(true),
          focusing(false),
...
void InteractiveButtonBase::setFontSize(int f) {
    ...
    if (text_dynamic_size) {
        ...
        setMinimumSize(fms.horizontalAdvance(text) + fore_paddings.left + fore_paddings.right,
                       fms.lineSpacing() + fore_paddings.top + fore_paddings.bottom);
    }
...
void InteractiveButtonBase::simulateStatePress(bool s, bool a) {
    ...
    mousePressEvent(
            new QMouseEvent(QMouseEvent::Type::None, QPoint(width() / 2, height() / 2), Qt::LeftButton, Qt::NoButton,
                            Qt::NoModifier));
...
void InteractiveButtonBase::discardHoverPress(bool force) {
    ...
    if (pressing) {
        mouseReleaseEvent(new QMouseEvent(QMouseEvent::Type::None, QPoint(width() / 2, height() / 2), Qt::LeftButton,
...
void InteractiveButtonBase::paintEvent(QPaintEvent *event) {
    ...
    if (fore_enabled /*针对按钮设置*/ && show_foreground /*针对动画设置*/) {
        ...
        // 绘制额外内容（可能被前景覆盖）
        if (paint_addin.enable) {
            int l = fore_paddings.left, t = fore_paddings.top, r = width() - fore_paddings.right, b =
                    height() - fore_paddings.bottom;
        ...
        } else if (model == IconText || model == PixmapText) // 强制左对齐；左图标中文字
        {
            ...
            icon_rect.moveTo(icon_rect.left() - quick_sqrt(long(offset_pos.x())),
                             icon_rect.top() - quick_sqrt(long(offset_pos.y())));
...
QIcon::Mode InteractiveButtonBase::getIconMode() {
    return isEnabled() ? (getState() ? QIcon::Selected : (hovering || pressing ? QIcon::Active : QIcon::Normal))
                       : QIcon::Disabled;
}
...
void InteractiveButtonBase::anchorTimeOut() {
    ...
    if (water_animation) {
        for (int i = 0; i < waters.size(); i++) {
            ...
            {
                water.progress = static_cast<int>(100 -
                                                  100 * (timestamp - water.finish_timestamp) / water_finish_duration);
                if (water.progress <= 0) {
                    waters.removeAt(i--);
            ...
            } else // 正在出现状态
            {
                ...
                } else // 动画中的
                {
                    if (water.release_timestamp) // 鼠标已经松开了
                    {
                        water.progress = static_cast<int>(
                                100 * (water.release_timestamp - water.press_timestamp) / water_press_duration +
                                100 * (timestamp - water.release_timestamp) / water_release_duration);
                    } else // 鼠标一直按下
                    {
                        water.progress = static_cast<int>(100 * (timestamp - water.press_timestamp) /
                                                          water_press_duration);
                    }
