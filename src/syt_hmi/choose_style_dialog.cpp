...
ChooseStyleDialog::ChooseStyleDialog(QWidget *parent)
    : QDialog(parent), ui(new Ui::ChooseStyleDialog) {
  ui->setupUi(this);
  ...
  ui->style_path_btn->setStyleSheet(
  ...
  ui->icon_label->setPixmap(
      QPixmap(":m_icon/icon/style_layer.png")
          .scaled(80, 80, Qt::AspectRatioMode::KeepAspectRatio,
  ...
  // 设置ListView的显示模式
  ui->style_file_list_view->setRootIndex(
      style_file_model_.index(style_directory_));
  ui->style_file_list_view->show();

  waiting_spinner_widget_ = new WaitingSpinnerWidget(this);

  connect(ui->style_path_btn, SIGNAL(clicked()), this,
          SLOT(slotSetStylePath()));
  connect(ui->confirm_btn, &QPushButton::clicked, [=]() {
    waiting_spinner_widget_->start();
    emit signSetCurrentStyle(
        ui->style_path_line_edit->text(),
        ui->style_file_list_view->currentIndex().data().toString());
  });
  connect(ui->return_btn, &QPushButton::clicked, [=]() { this->reject(); });

  connect(delete_act_, &QAction::triggered, this,
          &ChooseStyleDialog::deleteStyleFile);
  connect(ui->style_file_list_view, &QListView::customContextMenuRequested,
          this, &ChooseStyleDialog::showContextMenu);
}
